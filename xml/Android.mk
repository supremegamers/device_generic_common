#
# Copyright (C) 2022 BlissLabs
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#      http://www.apache.org/licenses/LICENSE-2.0
#
LOCAL_PATH := $(call my-dir)

include $(CLEAR_VARS)
LOCAL_MODULE := pc.xml
LOCAL_MODULE_TAGS := optional
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_RELATIVE_PATH := hidden_xml
LOCAL_SRC_FILES := pc.xml
LOCAL_POST_INSTALL_CMD := mkdir -p $(TARGET_OUT)/etc/permissions;
LOCAL_POST_INSTALL_CMD += touch $(TARGET_OUT)/etc/permissions/pc.xml
include $(BUILD_PREBUILT)

include $(CLEAR_VARS)
LOCAL_MODULE := hpe.xml
LOCAL_MODULE_TAGS := optional
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_RELATIVE_PATH := hidden_xml
LOCAL_SRC_FILES := hpe.xml
LOCAL_POST_INSTALL_CMD := mkdir -p $(TARGET_OUT)/etc/sysconfig;
LOCAL_POST_INSTALL_CMD += touch $(TARGET_OUT)/etc/sysconfig/hpe.xml
include $(BUILD_PREBUILT)

